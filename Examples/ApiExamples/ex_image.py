...
class ExImage(ApiExampleBase):
    ...
    def test_from_file(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.FromFile.docx')
    ...
    def test_from_url(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.FromUrl.docx')
    ...
    def test_create_floating_page_center(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.CreateFloatingPageCenter.docx')
        shape = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape()
    ...
    def test_create_floating_position_size(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.CreateFloatingPositionSize.docx')
        shape = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape()
        test_util.TestUtil.verify_image_in_shape(400, 400, aw.drawing.ImageType.JPEG, shape)
        self.assertEqual(aw.drawing.WrapType.NONE, shape.wrap_type)
        self.assertEqual(aw.drawing.RelativeHorizontalPosition.PAGE, shape.relative_horizontal_position)
        self.assertEqual(aw.drawing.RelativeVerticalPosition.PAGE, shape.relative_vertical_position)
        self.assertEqual(100, shape.left)
        self.assertEqual(80, shape.top)
        self.assertEqual(125, shape.height)
        self.assertEqual(125, shape.width)
        self.assertEqual(shape.top + shape.height, shape.bottom)
        self.assertEqual(shape.left + shape.width, shape.right)
    ...
    def test_insert_image_with_hyperlink(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.InsertImageWithHyperlink.docx')
        shape = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape()
        self.assertEqual('https://forum.aspose.com/', shape.href)
        test_util.TestUtil.verify_image_in_shape(400, 400, aw.drawing.ImageType.JPEG, shape)
    ...
    def test_create_linked_image(self):
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.CreateLinkedImage.Embedded.docx')
        shape = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape()
        test_util.TestUtil.verify_image_in_shape(1600, 1600, aw.drawing.ImageType.WMF, shape)
        self.assertEqual(aw.drawing.WrapType.INLINE, shape.wrap_type)
        self.assertEqual('', shape.image_data.source_full_name.replace('%20', ' '))
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Image.CreateLinkedImage.Linked.docx')
        shape = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape()
        test_util.TestUtil.verify_image_in_shape(0, 0, aw.drawing.ImageType.WMF, shape)
        self.assertEqual(aw.drawing.WrapType.INLINE, shape.wrap_type)
