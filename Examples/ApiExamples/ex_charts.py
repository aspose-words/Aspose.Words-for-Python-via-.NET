...
class ExCharts(ApiExampleBase):
    ...
    def test_date_time_values(self):
        ...
        import datetime
        import aspose.words as aw
        import aspose.words.drawing.charts as charts
        #ExStart
        #ExFor:AxisBound
        ...
        #ExEnd
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Charts.DateTimeValues.docx')
        chart = doc.get_child(aw.NodeType.SHAPE, 0, True).as_shape().chart
        self.assertEqual(aw.drawing.charts.AxisBound(to_ole_autodate(datetime.datetime(2017, 11, 5))), chart.axis_x.scaling.minimum)
        self.assertEqual(aw.drawing.charts.AxisBound(to_ole_autodate(datetime.datetime(2017, 12, 3))), chart.axis_x.scaling.maximum)
        self.assertEqual(aw.drawing.charts.AxisTimeUnit.DAYS, chart.axis_x.base_time_unit)
        self.assertEqual(7, chart.axis_x.major_unit)
        self.assertEqual(1, chart.axis_x.minor_unit)
        ...
        self.assertEqual(100, chart.axis_y.major_unit)
        self.assertEqual(50, chart.axis_y.minor_unit)
    ...
    #ExFor:ChartDataLabel.clear_format
    #ExFor:ChartDataLabelCollection
    #ExFor:ChartDataLabelCollection.show_data_labels_range
    ...
    def test_chart_data_point(self):
        ...
        #ExFor:ChartSeries.invert_if_negative
        #ExFor:ChartDataPoint
        #ExFor:ChartDataPoint.format
        #ExFor:ChartDataPoint.clear_format
        ...
        #ExFor:ChartDataPoint.invert_if_negative
    ...
    def test_chart_series_collection(self):
        ...
        #ExFor:ChartType
        #ExFor:Chart.series
        #ExFor:ChartSeriesCollection.add_date(str,List[datetime],List[float])
        #ExFor:ChartSeriesCollection.add_double(str,List[float],List[float])
        #ExFor:ChartSeriesCollection.add_bubbles(str,List[float],List[float],List[float])
    ...
    def _append_chart(builder, chart_type, width, height):
        ...
        chart = chart_shape.chart
        chart.series.clear()
        assert chart.series.count == 0  #ExSkip
        return chart
    #ExEnd
    ...
    def test_axis_display_unit(self):
        from api_example_base import ApiExampleBase, ARTIFACTS_DIR
        import aspose.words as aw
        from aspose.words.drawing.charts import ChartType, AxisTickMark, AxisBound, AxisBuiltInUnit
        from aspose.words import ParagraphAlignment

        class ExChartAxisDisplayUnit(ApiExampleBase):
            ...
            def test_axis_display_unit(self):
                ...
                self.assertEqual(AxisBuiltInUnit.CUSTOM, axis.display_unit.unit)  #ExSkip
                ...
                self.assertEqual(AxisTickMark.INSIDE, axis.major_tick_mark)
                self.assertEqual(AxisTickMark.INSIDE, axis.minor_tick_mark)
                ...
                self.assertEqual(1, axis.tick_labels.spacing)
                self.assertEqual(ParagraphAlignment.RIGHT, axis.tick_labels.alignment)
                self.assertEqual(AxisBuiltInUnit.CUSTOM, axis.display_unit.unit)
                self.assertEqual(1000000, axis.display_unit.custom_unit)
                axis = shape.chart.axis_y
                self.assertEqual(AxisTickMark.CROSS, axis.major_tick_mark)
                self.assertEqual(AxisTickMark.OUTSIDE, axis.minor_tick_mark)
                self.assertEqual(10, axis.major_unit)
