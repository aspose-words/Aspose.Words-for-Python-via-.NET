...
class ExComment(ApiExampleBase):
    ...
    def test_utc_date_time(self):
        ...
        comment = aw.Comment(doc=doc, author='John Doe', initial='J.D.', date_time=date_time)
        comment.set_text('My comment.')
        builder.current_paragraph.append_child(comment)
        doc.save(file_name=ARTIFACTS_DIR + 'Comment.UtcDateTime.docx')
        doc = aw.Document(file_name=ARTIFACTS_DIR + 'Comment.UtcDateTime.docx')
        comment = doc.get_child(aw.NodeType.COMMENT, 0, True).as_comment()
        # DateTimeUtc returns data without milliseconds
        assert date_time.strftime('%Y-%m-%d %I:%M:%S') == comment.date_time_utc.strftime('%Y-%m-%d %I:%M:%S')